...
describe('Player', () => {
    ...
    describe('drop()', () => {
        ...
        test('throws error if player has no such item', () => {
            expect(() => target.drop('Bad')).toThrowError(/Player does not have/);
        });
    });
    ...
    describe('get currentConditions()', () => {
        ...
        test('initial state is correct', () => {
            expect(target.currentConditions).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({ atLoc: 'Foyer' })
                ])
            );
        });
        ...
        test('state is correct after move', () => {
            ...
            expect(target.currentConditions).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({ atLoc: 'Room' })
                ])
            );
        });
        ...
        test('state is correct after pickups', () => {
            ...
            expect(res).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({ hasItem: 'Skull' })
                ])
            );

            expect(res).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({ hasItem: 'Apple' })
                ])
            );

            expect(res).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({ hasItem: 'Pop' })
                ])
            );
    ...
    describe('query()', () => {
        ...
        test('throws error if any of the conditions are invalid', () => {
            expect(
                () => target.query({testProp: 'testValue'})
        ...
        describe('hasItem', () => {
            ...
            test('returns true for valid item', () => {
                expect(
                    target.query({hasItem: 'Skull'})
                ).toEqual(true);
            });

            test('returns false for invalid item', () => {
                expect(
                    target.query({hasItem: 'BAD'})
                ).toEqual(false);
            });
        });
        ...
        describe('atLocation', () => {
            ...
            test('returns true for current location', () => {
                expect(
                    target.query({atLocation: 'Room'})
                ).toEqual(true);
            });

            test('returns false for any other location', () => {
                expect(
                    target.query({atLocation: 'Foyer'})
                ).toEqual(false);
            });
        });
        ...
        describe('searchedLocation', () => {
            ...
            test('returns true if player has searched there', () => {
                expect(
                    target.query({hasSearched: 'Foyer'})
                ).toEqual(true);
            });

            test('returns false if player has not searched there', () => {
                expect(
                    target.query({hasSearched: 'Room'})
                ).toEqual(false);
            });
        });
        ...
        describe('exploredLocation', () => {
            ...
            test('returns true if player has explored there', () => {
                expect(
                    target.query({hasExplored: 'Foyer'})
                ).toEqual(true);
            });

            test('returns false if player has not explored there', () => {
                expect(
                    target.query({hasExplored: 'Room'})
                ).toEqual(false);
            });
        });
        ...
        describe('visitedLocation', () => {
            ...
            test('returns true if player has visited there', () => {
                expect(
                    target.query({hasVisited: 'Room'})
                ).toEqual(true);
            });

            test('returns false if players have not visited there', () => {
                expect(
                    target.query({hasVisited: 'BAD'})
                ).toEqual(false);
            });
        });
        ...
        describe('hasHealth', () => {
            test('returns true if health is equal to arg', () => {
                expect(
                    target.query({hasHealth: 100})
                ).toEqual(true);
            });

            test('returns false if health is not equal to arg', () => {
                expect(
                    target.query({hasHealth: 50})
                ).toEqual(false);
            });
        });
        ...
        describe('hasHealthGreaterThan', () => {
            test('returns true if health >= arg', () => {
                expect(
                    target.query({hasHealthGreaterThan: 100})
                ).toEqual(true);
            });

            test('returns false if health < arg', () => {
                expect(
                    target.query({hasHealthGreaterThan: 150})
                ).toEqual(false);
            });
        });
        ...
        describe('hasHealthLessThan', () => {
            test('returns true if health <= arg', () => {
                expect(
                    target.query({hasHealthLessThan: 100})
                ).toEqual(true);
            });

            test('returns false if healh > arg', () => {
                expect(
                    target.query({hasHealthLessThan: 50})
                ).toEqual(false);
            });
        });
        ...
        describe('hasBattery', () => {
            test('returns true if battery is equal to arg', () => {
                expect(
                    target.query({hasBattery: 100})
                ).toEqual(true);
            });

            test('returns false if battery is not equal to arg', () => {
                expect(
                    target.query({hasBattery: 50})
                ).toEqual(false);
            });
        });
        ...
        describe('hasBatteryGreaterThan', () => {
            test('returns true if battery >= arg', () => {
                expect(
                    target.query({hasBatteryGreaterThan: 50})
                ).toEqual(true);
            });

            test('returns false if battery < arg', () => {
                expect(
                    target.query({hasBatteryGreaterThan: 150})
                ).toEqual(false);
            });
        });
        ...
        describe('hasBatteryLessThan', () => {
            test('returns true if battery <= arg', () => {
                expect(
                    target.query({hasBatteryLessThan: 150})
                ).toEqual(true);
            });

            test('returns false if battery > arg', () => {
                expect(
                    target.query({hasBatteryLessThan: 50})
                ).toEqual(false);
            });
        });
    });
